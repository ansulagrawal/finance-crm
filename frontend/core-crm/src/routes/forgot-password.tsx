  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        // either way (a burned request must not be distinguishable from a
        // wrong guess). So the copy can't tell the user which happened —
        // it points at the one action that always works instead: request a
        // fresh OTP.
        const isBadOtp =
          error instanceof ApiError &&
