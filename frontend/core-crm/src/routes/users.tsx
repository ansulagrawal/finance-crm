...
function NewUserForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  // `users.mobile` is NOT NULL with no default, so the API requires exactly
  // 10 digits. Checking it here turns a post-submit 400 ("mobile must be a
  // string, mobile must be 10 digits" — both fire when the field is absent)
      ...
      <div className='flex flex-col gap-1'>
        ...
        <Input
          ...
          onChange={(event) =>
            setForm((f) => ({
              ...f,
              // Strip as you type: the API takes digits only, and pasting a
              // number with spaces or a +91 prefix is the common way to fail.
...
function UsersPage() {
  ...
  return (
    <>
      <div className='flex items-center justify-between'>
        <div>
          <h1 className='font-semibold text-2xl text-primary'>Users</h1>
