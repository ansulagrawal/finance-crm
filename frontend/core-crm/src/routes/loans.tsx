...
function LoansPage() {
  ...
  // The "all disbursal leads" view has no role-based stage scoping on the
  // backend (`listQueue` only restricts stageCode for roles present in
  // `QUEUE_ROLE_SCOPES`) — a single `leadStatusId` is the only server-side
  // filter available, so browsing outside "My queue" requires picking one
  // of the four disbursal statuses rather than seeing all of them at once.
  // Gated on `queueDefaultApplied` so this can't race ahead of the "default
  // to my queue" effect above and lock in a specific status before we know
  // whether the user actually lands on "My queue" (which should stay ALL).
  useEffect(() => {
    if (
      queueDefaultApplied &&
      !myQueue &&
  ...
  // `listLeadsQueue` unions the stage scopes of *every* role the acting
  // user holds (`QUEUE_ROLE_SCOPES` in core-api's leads.service.ts) — a
  // user who is both DS1 and, say, CR1 gets CR1's screening-stage leads
  // mixed into "My queue" too. Filter to the disbursal stage codes
  // client-side so this page only ever shows disbursal-pipeline leads,
  // regardless of the acting user's other roles.
  ...
  // Warm the per-row loan cache in parallel so the columns don't populate
  // one row at a time as `LoanCells`/`OutstandingCell` mount.
  useQueries({
  ...
  return (
    <>
      <div>
        <h1 className='font-semibold text-2xl text-primary'>Loans</h1>
