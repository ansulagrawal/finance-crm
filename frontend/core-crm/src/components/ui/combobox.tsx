...
/** Single searchable dropdown: types and picks in one control, backed by an
 * externally-owned `search` string so callers can debounce/query a server
 * (see `listUsers`-backed pickers in `users.tsx`/`collection-buckets.tsx`).
 * Built on the same `PopoverPrimitive` already used by `MultiSelect` rather
 * than adding a combobox library — Radix's `Select` has no room for a
 * filter input inside its dropdown, but a plain `Input` + option list does. */
