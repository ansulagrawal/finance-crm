...
/**
 * Whole-page fallback, wired once as the router's `defaultErrorComponent` /
 * `defaultNotFoundComponent` (see `main.tsx`) so every route gets it without
 * opting in.
 *
 * Without it, TanStack Router falls back to its own bare "Something went
 * wrong! / Hide Error" overlay, which shows a raw exception message
 * ("Cannot read properties of null (reading 'useContext')") to a loan officer
 * who can do nothing with it.
 *
 * The exception text is still reachable, behind a collapsed `<details>`:
 * hiding it entirely just means a screenshot of a bug report says nothing.
 */
function Shell({
  icon,
  title,
