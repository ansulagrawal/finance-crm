...
/** Customer (KYC), Employment, and References editors for a lead —
 * `PUT`/`POST` against the lead's own sub-resources, independent of lead
 * creation itself (no atomic multi-step transaction on the backend). Shared
 * between the lead detail page (post-creation editing) and the new-lead
 * intake wizard (`routes/index.tsx`), which calls them with the
 * just-created lead's id as each of its later steps. */
/** One row per email address the customer has on file — personal (`email`)
 * or office (`alternateEmail`, confirmed as legacy's `alternate_email`
 * field). Each offers independent domain and deliverability verification
 * (Signzy `domainVerificationLite` / `email/verificationV2`), since legacy
 * only ever confirmed the office-email capture point, not an automatic
 * verification trigger — so this stays an explicit per-email action. */
